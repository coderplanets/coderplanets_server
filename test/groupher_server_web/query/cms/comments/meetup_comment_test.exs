defmodule GroupherServer.Test.Query.Comments.MeetupComment do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(user_conn guest_conn meetup user user2)a}
  end

  describe "[baisc article meetup comment]" do
    @query """
    query($id: ID!) {
      meetup(id: $id) {
        id
    ...
    test "guest user can get comment participants after comment created",
         ~m(guest_conn meetup user user2)a do
      total_count = 5
      thread = :meetup

      Enum.reduce(1..total_count, [], fn _, acc ->
        {:ok, comment} = CMS.create_comment(thread, meetup.id, mock_comment(), user)

        acc ++ [comment]
      end)

      {:ok, _} = CMS.create_comment(thread, meetup.id, mock_comment(), user2)

      variables = %{id: meetup.id}
      results = guest_conn |> query_result(@query, variables, "meetup")
    ...
    test "list comments with default replies-mode", ~m(guest_conn meetup user user2)a do
      total_count = 10
      page_size = 20
      thread = :meetup
      ...
      all_comments =
        Enum.reduce(1..total_count, [], fn i, acc ->
          {:ok, comment} =
            CMS.create_comment(thread, meetup.id, mock_comment("comment #{i}"), user)
      ...
      variables = %{id: meetup.id, thread: "MEETUP", filter: %{page: 1, size: page_size}}
    ...
    test "timeline-mode paged comments", ~m(guest_conn meetup user user2)a do
      total_count = 3
      page_size = 20
      thread = :meetup
    ...
    test "comment should have reply_to content if need", ~m(guest_conn meetup user user2)a do
      ...
      Enum.reduce(0..total_count, [], fn i, acc ->
        {:ok, comment} = CMS.create_comment(thread, meetup.id, mock_comment("comment #{i}"), user)

        acc ++ [comment]
      end)

      {:ok, parent_comment} =
        CMS.create_comment(:meetup, meetup.id, mock_comment("parent_comment"), user)
    ...
    test "guest user can get paged comment for meetup", ~m(guest_conn meetup user)a do
      total_count = 30
      thread = :meetup

      Enum.reduce(1..total_count, [], fn _, acc ->
        {:ok, value} = CMS.create_comment(thread, meetup.id, mock_comment(), user)

        acc ++ [value]
      end)

      variables = %{id: meetup.id, thread: "MEETUP", filter: %{page: 1, size: 10}}
      results = guest_conn |> query_result(@query, variables, "pagedComments")
    ...
    test "guest user can get paged comment with floor it", ~m(guest_conn meetup user)a do
      total_count = 5
      thread = :meetup
      page_size = 10
    ...
    test "the comments is loaded in default asc order", ~m(guest_conn meetup user)a do
      page_size = 10
      thread = :meetup

      {:ok, comment} = CMS.create_comment(thread, meetup.id, mock_comment(), user)
      Process.sleep(1000)
      {:ok, _comment2} = CMS.create_comment(thread, meetup.id, mock_comment(), user)
      Process.sleep(1000)
      {:ok, comment3} = CMS.create_comment(thread, meetup.id, mock_comment(), user)
    ...
    test "the comments can be loaded in desc order in replies-mode",
      ...
      Process.sleep(1000)
      {:ok, comment2} = CMS.create_comment(thread, meetup.id, mock_comment(), user)
    ...
    test "article author upvote a comment can get is_article_author and/or is_article_author_upvoted flag",
      ...
      {:ok, author_comment} = CMS.create_comment(thread, meetup.id, mock_comment(), author_user)
    ...
    test "user make emotion can get paged comment with emotions has_motioned field",
         ~m(user_conn meetup user user2)a do
      total_count = 10
      page_size = 12
      thread = :meetup
    ...
    test "comment should have viewer has upvoted flag", ~m(user_conn meetup user)a do
      total_count = 10
      page_size = 12
      thread = :meetup
  ...
  describe "paged participants" do
    ...
    test "guest user can get paged participants", ~m(guest_conn meetup user)a do
      total_count = 30
      page_size = 10
      thread = "MEETUP"
      ...
      variables = %{id: meetup.id, thread: thread, filter: %{page: 1, size: page_size}}
  ...
  describe "paged replies" do
    ...
    test "guest user can get paged replies", ~m(guest_conn meetup user user2)a do
      ...
      author_user = meetup.author.user
      {:ok, parent_comment} = CMS.create_comment(thread, meetup.id, mock_comment(), user)
