defmodule GroupherServer.Test.Mutation.ArticleCommunity.Works do
  ...
  setup do
    ...
    {:ok, ~m(user_conn guest_conn owner_conn community works)a}
  end

  describe "[mirror/unmirror/move works to/from community]" do
    ...
    """
    test "auth user can mirror a works to other community", ~m(works)a do
      passport_rules = %{"works.community.mirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)

      {:ok, community} = db_insert(:community)
      variables = %{id: works.id, thread: "WORKS", communityId: community.id}
      rule_conn |> mutation_result(@mirror_article_query, variables, "mirrorArticle")
      {:ok, found} = ORM.find(Works, works.id, preload: :communities)

      assoc_communities = found.communities |> Enum.map(& &1.id)
      assert community.id in assoc_communities
    end

    test "unauth user cannot mirror a works to a community", ~m(user_conn guest_conn works)a do
      {:ok, community} = db_insert(:community)
      variables = %{id: works.id, thread: "WORKS", communityId: community.id}
    ...
    test "auth user can mirror multi works to other communities", ~m(works)a do
      passport_rules = %{"works.community.mirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      variables = %{id: works.id, thread: "WORKS", communityId: community2.id}
      rule_conn |> mutation_result(@mirror_article_query, variables, "mirrorArticle")
    ...
    test "auth user can unmirror works to a community", ~m(works)a do
      passport_rules = %{"works.community.mirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      variables2 = %{id: works.id, thread: "WORKS", communityId: community2.id}
      ...
      passport_rules = %{"works.community.unmirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
    ...
    """
    test "auth user can move works to other community", ~m(works)a do
      passport_rules = %{"works.community.mirror" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      variables = %{id: works.id, thread: "WORKS", communityId: community.id}
      rule_conn |> mutation_result(@mirror_article_query, variables, "mirrorArticle")
      {:ok, found} = ORM.find(Works, works.id, preload: [:original_community, :communities])
      assoc_communities = found.communities |> Enum.map(& &1.id)
      assert community.id in assoc_communities

      passport_rules = %{"works.community.move" => true}
      rule_conn = simu_conn(:user, cms: passport_rules)
