defmodule GroupherServer.Test.Mutation.ArticleTags.DrinkTag do
  @moduledoc false
  ...
  setup do
    ...
    {:ok,
     ~m(user_conn guest_conn owner_conn community drink article_tag_attrs article_tag_attrs2 user)a}
  end

  describe "[mutation drink tag]" do
    @set_tag_query """
    ...
    test "auth user can set a valid tag to drink", ~m(community drink article_tag_attrs user)a do
      {:ok, article_tag} = CMS.create_article_tag(community, :drink, article_tag_attrs, user)

      passport_rules = %{community.title => %{"drink.article_tag.set" => true}}
      ...
      {:ok, found} = ORM.find(Drink, drink.id, preload: :article_tags)
    ...
    test "can unset tag to a drink",
      ...
      passport_rules = %{community.title => %{"drink.article_tag.unset" => true}}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      assoc_tags = drink.article_tags |> Enum.map(& &1.id)
