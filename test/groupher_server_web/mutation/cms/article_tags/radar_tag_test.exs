defmodule GroupherServer.Test.Mutation.ArticleTags.RadarTag do
  @moduledoc false
  ...
  setup do
    ...
    {:ok,
     ~m(user_conn guest_conn owner_conn community radar article_tag_attrs article_tag_attrs2 user)a}
  end

  describe "[mutation radar tag]" do
    @set_tag_query """
    ...
    test "auth user can set a valid tag to radar", ~m(community radar article_tag_attrs user)a do
      {:ok, article_tag} = CMS.create_article_tag(community, :radar, article_tag_attrs, user)

      passport_rules = %{community.title => %{"radar.article_tag.set" => true}}
      ...
      {:ok, found} = ORM.find(Radar, radar.id, preload: :article_tags)
    ...
    test "can unset tag to a radar",
      ...
      passport_rules = %{community.title => %{"radar.article_tag.unset" => true}}
      rule_conn = simu_conn(:user, cms: passport_rules)
      ...
      assoc_tags = radar.article_tags |> Enum.map(& &1.id)
