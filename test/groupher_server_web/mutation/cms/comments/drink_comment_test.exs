defmodule GroupherServer.Test.Mutation.Comments.DrinkComment do
  ...
  setup do
    ...
    {:ok, ~m(user_conn user guest_conn owner_conn community drink)a}
  end
  ...
  describe "[article comment CURD]" do
    ...
    """
    test "write article comment to a exsit drink", ~m(drink user_conn)a do
      variables = %{thread: "DRINK", id: drink.id, body: mock_comment()}
    ...
    """
    test "login user can reply to a comment", ~m(drink user user_conn)a do
    ...
    test "only owner can update a exsit comment",
         ~m(drink user guest_conn user_conn owner_conn)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
  ...
  describe "[article comment upvote]" do
    ...
    test "login user can upvote a exsit drink comment", ~m(drink user guest_conn user_conn)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      variables = %{id: comment.id}
    ...
    test "login user can undo upvote a exsit drink comment",
         ~m(drink user guest_conn user_conn)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      variables = %{id: comment.id}
  ...
  describe "[article comment emotion]" do
    ...
    """
    test "login user can emotion to a comment", ~m(drink user user_conn)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    """
    test "login user can undo emotion to a comment", ~m(drink user owner_conn)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
  ...
  describe "[article comment lock/unlock]" do
    ...
    mutation($id: ID!, $communityId: ID!){
      lockDrinkComment(id: $id, communityId: $communityId) {
        id
      }
    }
    """

    test "can lock a drink's comment", ~m(community drink)a do
      variables = %{id: drink.id, communityId: community.id}
      passport_rules = %{community.raw => %{"drink.lock_comment" => true}}
      rule_conn = simu_conn(:user, cms: passport_rules)

      result = rule_conn |> mutation_result(@query, variables, "lockDrinkComment")
      assert result["id"] == to_string(drink.id)

      {:ok, drink} = ORM.find(Drink, drink.id)
      assert drink.meta.is_comment_locked
    end

    test "unauth user  fails", ~m(guest_conn community drink)a do
      variables = %{id: drink.id, communityId: community.id}
    ...
    @query """
    mutation($id: ID!, $communityId: ID!){
      undoLockDrinkComment(id: $id, communityId: $communityId) {
        id
      }
    }
    """

    test "can undo lock a drink's comment", ~m(community drink)a do
      {:ok, _} = CMS.lock_article_comments(:drink, drink.id)
      {:ok, drink} = ORM.find(Drink, drink.id)
      assert drink.meta.is_comment_locked

      variables = %{id: drink.id, communityId: community.id}
      passport_rules = %{community.raw => %{"drink.undo_lock_comment" => true}}
      rule_conn = simu_conn(:user, cms: passport_rules)

      result = rule_conn |> mutation_result(@query, variables, "undoLockDrinkComment")
      assert result["id"] == to_string(drink.id)

      {:ok, drink} = ORM.find(Drink, drink.id)
      assert not drink.meta.is_comment_locked
    end

    test "unauth user undo fails", ~m(guest_conn community drink)a do
      variables = %{id: drink.id, communityId: community.id}
  ...
  describe "[article comment pin/unPin]" do
    ...
    test "can pin a drink's comment", ~m(owner_conn drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "unauth user fails.", ~m(guest_conn drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      variables = %{id: comment.id}
    ...
    test "can undo pin a drink's comment", ~m(owner_conn drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "unauth user undo fails.", ~m(guest_conn drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _} = CMS.pin_comment(comment.id)
      variables = %{id: comment.id}
