defmodule GroupherServer.Test.CMS.Hooks.NotifyRadar do
  ...
  setup do
    ...
    {:ok, ~m(user2 user3 radar comment)a}
  end

  describe "[upvote notify]" do
    test "upvote hook should work on radar", ~m(user2 radar)a do
      ...
      {:ok, notifications} =
        Delivery.fetch(:notification, radar.author.user, %{page: 1, size: 20})

      assert notifications.total_count == 1

      notify = notifications.entries |> List.first()
      assert notify.action == "UPVOTE"
      assert notify.article_id == radar.id
      assert notify.thread == "RADAR"
      assert notify.user_id == radar.author.user.id
      assert user_exist_in?(user2, notify.from_users)
    end

    test "upvote hook should work on radar comment", ~m(user2 radar comment)a do
    ...
    test "undo upvote hook should work on radar", ~m(user2 radar)a do
      {:ok, radar} = preload_author(radar)
    ...
    test "undo upvote hook should work on radar comment", ~m(user2 comment)a do
      {:ok, comment} = CMS.upvote_comment(comment.id, user2)
  ...
  describe "[collect notify]" do
    test "collect hook should work on radar", ~m(user2 radar)a do
      {:ok, radar} = preload_author(radar)

      {:ok, _} = CMS.collect_article(:radar, radar.id, user2)
      Hooks.Notify.handle(:collect, radar, user2)
    ...
    test "undo collect hook should work on radar", ~m(user2 radar)a do
      {:ok, radar} = preload_author(radar)

      {:ok, _} = CMS.upvote_article(:radar, radar.id, user2)
      Hooks.Notify.handle(:collect, radar, user2)

      {:ok, _} = CMS.undo_upvote_article(:radar, radar.id, user2)
      Hooks.Notify.handle(:undo, :collect, radar, user2)
  ...
  describe "[comment notify]" do
    test "radar author should get notify after some one comment on it", ~m(user2 radar)a do
      {:ok, radar} = preload_author(radar)

      {:ok, comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user2)
    ...
    test "radar comment author should get notify after some one reply it",
         ~m(user2 user3 radar)a do
      {:ok, radar} = preload_author(radar)
