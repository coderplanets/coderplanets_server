defmodule GroupherServer.Test.CMS.Comments.WorksCommentReplies do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(user user2 works)a}
  end

  describe "[basic article comment replies]" do
    test "exsit comment can be reply", ~m(works user user2)a do
    ...
    test "deleted comment can not be reply", ~m(works user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "multi reply should belong to one parent comment", ~m(works user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "comment replies only contains @max_parent_replies_count replies", ~m(works user)a do
    ...
    test "replyed user should appear in article comment participants", ~m(works user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _} = CMS.reply_comment(parent_comment.id, mock_comment(), user2)

      {:ok, article} = ORM.find(Works, works.id)

      assert exist_in?(user, article.comments_participants)
      assert exist_in?(user2, article.comments_participants)
    end

    test "replies count should inc by 1 after got replyed", ~m(works user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
