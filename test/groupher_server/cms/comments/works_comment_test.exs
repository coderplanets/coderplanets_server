defmodule GroupherServer.Test.CMS.Comments.WorksComment do
  @moduledoc false
  ...
  alias CMS.Model.{Comment, PinnedComment, Embeds, Works}
  ...
  setup do
    ...
    {:ok, ~m(community user user2 works)a}
  end

  describe "[basic article comment]" do
    test "works are supported by article comment.", ~m(user works)a do
      {:ok, works_comment_1} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, works_comment_2} = CMS.create_comment(:works, works.id, mock_comment(), user)

      {:ok, works} = ORM.find(Works, works.id, preload: :comments)

      assert exist_in?(works_comment_1, works.comments)
      assert exist_in?(works_comment_2, works.comments)
    end

    test "comment should have default meta after create", ~m(user works)a do
    ...
    test "create comment should update active timestamp of works", ~m(user works)a do
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, works} = ORM.find(Works, works.id, preload: :comments)

      assert not is_nil(works.active_at)
      assert works.active_at > works.inserted_at
    end

    test "works author create comment will not update active timestamp", ~m(community user)a do
      works_attrs = mock_attrs(:works, %{community_id: community.id})
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
      {:ok, works} = ORM.find(Works, works.id, preload: [author: :user])

      Process.sleep(1000)

      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), works.author.user)
    ...
    test "old works will not update active after comment created", ~m(user)a do
      ...
      {:ok, works} = db_insert(:works, %{inserted_at: inserted_at})
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, works} = ORM.find(Works, works.id)

      assert works.active_at |> DateTime.to_date() == DateTime.utc_now() |> DateTime.to_date()
      ...
      assert works.active_at |> DateTime.to_unix() !== DateTime.utc_now() |> DateTime.to_unix()
    end

    test "comment can be updated", ~m(works user)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
  ...
  describe "[article comment floor]" do
    test "comment will have a floor number after created", ~m(user works)a do
      {:ok, works_comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, works_comment2} = CMS.create_comment(:works, works.id, mock_comment(), user)

      {:ok, works_comment} = ORM.find(Comment, works_comment.id)
      {:ok, works_comment2} = ORM.find(Comment, works_comment2.id)

      assert works_comment.floor == 1
      assert works_comment2.floor == 2
    end
  end

  describe "[article comment participator for works]" do
    test "works will have participator after comment created", ~m(user works)a do
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)

      {:ok, works} = ORM.find(Works, works.id)

      participator = List.first(works.comments_participants)
      assert participator.id == user.id
    end

    test "psot participator will not contains same user", ~m(user works)a do
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)

      {:ok, works} = ORM.find(Works, works.id)

      assert 1 == length(works.comments_participants)
    end

    test "recent comment user should appear at first of the psot participants",
         ~m(user user2 works)a do
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user2)
  ...
  describe "[article comment upvotes]" do
    test "user can upvote a works comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "article author upvote works comment will have flag", ~m(works user)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, author_user} = ORM.find(User, works.author.user.id)
    ...
    test "user upvote works comment will add id to upvoted_user_ids", ~m(works user)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "user undo upvote works comment will remove id from upvoted_user_ids",
         ~m(works user user2)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "user upvote a already-upvoted comment fails", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "upvote comment should inc the comment's upvotes_count", ~m(user user2 works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "user can undo upvote a works comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      CMS.upvote_comment(comment.id, user)
    ...
    test "user can undo upvote a works comment with no upvote", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, comment} = CMS.undo_upvote_comment(comment.id, user)
      assert 0 == comment.upvotes_count
  ...
  describe "[article comment fold/unfold]" do
    test "user can fold a comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, works} = ORM.find(Works, works.id)
      assert works.meta.folded_comment_count == 1
    end

    test "user can unfold a comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      ...
      {:ok, works} = ORM.find(Works, works.id)
      assert works.meta.folded_comment_count == 0
    end
  end

  describe "[article comment pin/unpin]" do
    test "user can pin a comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, pined_record} = PinnedComment |> ORM.find_by(%{works_id: works.id})
      assert pined_record.works_id == works.id
    end

    test "user can unpin a comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "pinned comments has a limit for each article", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
  ...
  describe "[article comment report/unreport]" do
    #
    # test "user can report a comment", ~m(user works)a do
    #   {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    #   {:ok, comment} = ORM.find(Comment, comment.id)
    ...
    #
    # test "user can unreport a comment", ~m(user works)a do
    #   {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "can undo a report with other user report it too", ~m(user user2 works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "report user < @report_threshold_for_fold will not fold comment", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "report user > @report_threshold_for_fold will cause comment fold", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(user works)a do
      total_count = 30
      page_size = 10
      thread = :works
      ...
      Enum.reduce(1..total_count, [], fn _, acc ->
        ...
        {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), new_user)
      ...
      {:ok, results} =
        CMS.paged_comments_participants(thread, works.id, %{page: 1, size: page_size})

      assert results |> is_valid_pagination?(:raw)
      assert results.total_count == total_count + 1
    end

    test "paged article comments folded flag should be false", ~m(user works)a do
      total_count = 30
      page_number = 1
      page_size = 35
    ...
    test "paged article comments should contains pinned comments at top position",
      ...
      {:ok, random_comment_1} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, random_comment_2} = CMS.create_comment(:works, works.id, mock_comment(), user)
    ...
    test "can loaded paged folded comment", ~m(user works)a do
      total_count = 10
      page_number = 1
      page_size = 20
      ...
      {:ok, paged_comments} =
        CMS.paged_folded_comments(:works, works.id, %{page: page_number, size: page_size})
  ...
  describe "[article comment delete]" do
    test "delete comment still exsit in paged list and content is gone", ~m(user works)a do
      total_count = 10
      page_number = 1
      page_size = 20
    ...
    test "delete comment still update article's comments_count field", ~m(user works)a do
      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      ...
      assert works.comments_count == 5

      {:ok, _} = CMS.delete_comment(comment)

      {:ok, works} = ORM.find(Works, works.id)
      assert works.comments_count == 4
    end

    test "delete comment still delete pinned record if needed", ~m(user works)a do
      total_count = 10
  ...
  describe "[article comment info]" do
    test "author of the article comment a comment should have flag", ~m(user works)a do
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), user)
      assert not comment.is_article_author

      author_user = works.author.user
      {:ok, comment} = CMS.create_comment(:works, works.id, mock_comment(), author_user)
      assert comment.is_article_author
    end
  end

  describe "[lock/unlock works comment]" do
    test "locked works can not be comment", ~m(user works)a do
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)
      {:ok, _} = CMS.lock_article_comments(:works, works.id)

      {:error, reason} = CMS.create_comment(:works, works.id, mock_comment(), user)
      assert reason |> is_error?(:article_comments_locked)

      {:ok, _} = CMS.undo_lock_article_comments(:works, works.id)
      {:ok, _} = CMS.create_comment(:works, works.id, mock_comment(), user)
    end

    test "locked works can not by reply", ~m(user works)a do
