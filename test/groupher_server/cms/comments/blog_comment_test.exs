defmodule GroupherServer.Test.CMS.Comments.BlogComment do
  ...
  describe "[basic article comment]" do
    test "blog are supported by article comment.", ~m(user blog)a do
      {:ok, blog_comment_1} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
      {:ok, blog_comment_2} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
    ...
    test "blog author create comment will not update active timestamp", ~m(community user)a do
      ...
      {:ok, _comment} = CMS.create_comment(:blog, blog.id, mock_comment(), blog.author.user)
  ...
  describe "[article comment floor]" do
    test "comment will have a floor number after created", ~m(user blog)a do
      {:ok, blog_comment} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
      {:ok, blog_comment2} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
  ...
  describe "[article comment participator for blog]" do
    test "blog will have participator after comment created", ~m(user blog)a do
      {:ok, _} = CMS.create_comment(:blog, blog.id, mock_comment(), user)

      {:ok, blog} = ORM.find(Blog, blog.id)

      participator = List.first(blog.article_comments_participants)
    ...
    test "psot participator will not contains same user", ~m(user blog)a do
      ...
      assert 1 == length(blog.article_comments_participants)
    end

    test "recent comment user should appear at first of the psot participants",
         ~m(user user2 blog)a do
      {:ok, _} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:blog, blog.id, mock_comment(), user2)
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(user blog)a do
      total_count = 30
      page_size = 10
      thread = :blog
    ...
    test "paged article comments should contains pinned comments at top position",
      ...
      {:ok, random_comment_1} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
      {:ok, random_comment_2} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
  ...
  describe "[article comment info]" do
    test "author of the article comment a comment should have flag", ~m(user blog)a do
      ...
      {:ok, comment} = CMS.create_comment(:blog, blog.id, mock_comment(), author_user)
  ...
  describe "[lock/unlock blog comment]" do
    test "locked blog can not be comment", ~m(user blog)a do
      ...
      {:error, reason} = CMS.create_comment(:blog, blog.id, mock_comment(), user)
