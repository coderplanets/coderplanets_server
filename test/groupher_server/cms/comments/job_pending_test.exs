defmodule GroupherServer.Test.CMS.Comments.JobPending do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(guest_conn community user job)a}
  end

  describe "[pending job comemnt flags]" do
    test "pending job comment can set/unset pending", ~m(job user)a do
    ...
    test "pending job-comment's meta should have info", ~m(job user)a do
      {:ok, comment} = CMS.create_comment(:job, job.id, mock_comment(), user)
      ...
      {:ok, _} =
        CMS.set_comment_illegal(comment.id, %{
          ...
          illegal_reason: ["some-reason"],
          illegal_words: ["some-word"],
          illegal_comments: ["/job/#{job.id}/comment/#{comment.id}"]
        })
      ...
      assert user.meta.illegal_comments == ["/job/#{job.id}/comment/#{comment.id}"]
