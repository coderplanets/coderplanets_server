defmodule GroupherServer.Test.CMS.Comments.JobComment do
  ...
  describe "[article comment floor]" do
    test "comment will have a floor number after created", ~m(user job)a do
      {:ok, job_comment} = CMS.create_comment(:job, job.id, mock_comment(), user)
      {:ok, job_comment2} = CMS.create_comment(:job, job.id, mock_comment(), user)

      {:ok, job_comment} = ORM.find(Comment, job_comment.id)
      {:ok, job_comment2} = ORM.find(Comment, job_comment2.id)
  ...
  describe "[article comment participator for job]" do
    test "job will have participator after comment created", ~m(user job)a do
      {:ok, _} = CMS.create_comment(:job, job.id, mock_comment(), user)
    ...
    test "recent comment user should appear at first of the psot participants",
      ...
      {:ok, _} = CMS.create_comment(:job, job.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:job, job.id, mock_comment(), user2)
  ...
  describe "[article comment upvotes]" do
    ...
    test "article author upvote job comment will have flag", ~m(job user)a do
      {:ok, comment} = CMS.create_comment(:job, job.id, mock_comment(), user)
      {:ok, author_user} = ORM.find(User, job.author.user.id)
  ...
  describe "[article comment fold/unfold]" do
    ...
    test "user can unfold a comment", ~m(user job)a do
      ...
      {:ok, job} = ORM.find(Job, job.id)
      assert job.meta.folded_comment_count == 0
    end
  end
  ...
  describe "[article comment pin/unpin]" do
    test "user can pin a comment", ~m(user job)a do
      ...
      {:ok, pined_record} = PinnedComment |> ORM.find_by(%{job_id: job.id})
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(user job)a do
      total_count = 30
      page_size = 10
      thread = :job
      ...
      Enum.reduce(1..total_count, [], fn _, acc ->
        ...
        {:ok, comment} = CMS.create_comment(:job, job.id, mock_comment(), new_user)
      ...
      {:ok, results} =
        CMS.paged_comments_participants(thread, job.id, %{page: 1, size: page_size})
    ...
    test "paged article comments should contains pinned comments at top position",
      ...
      {:ok, random_comment_1} = CMS.create_comment(:job, job.id, mock_comment(), user)
      {:ok, random_comment_2} = CMS.create_comment(:job, job.id, mock_comment(), user)
    ...
    test "can loaded paged folded comment", ~m(user job)a do
      ...
      {:ok, paged_comments} =
        CMS.paged_folded_comments(:job, job.id, %{page: page_number, size: page_size})
  ...
  describe "[article comment info]" do
    test "author of the article comment a comment should have flag", ~m(user job)a do
      ...
      {:ok, comment} = CMS.create_comment(:job, job.id, mock_comment(), author_user)
  ...
  describe "[lock/unlock job comment]" do
    test "locked job can not be comment", ~m(user job)a do
      ...
      {:error, reason} = CMS.create_comment(:job, job.id, mock_comment(), user)
      assert reason |> is_error?(:article_comments_locked)

      {:ok, _} = CMS.undo_lock_article_comments(:job, job.id)
      {:ok, _} = CMS.create_comment(:job, job.id, mock_comment(), user)
    end

    test "locked job can not by reply", ~m(user job)a do
      {:ok, parent_comment} = CMS.create_comment(:job, job.id, mock_comment(), user)
