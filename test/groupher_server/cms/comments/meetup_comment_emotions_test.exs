defmodule GroupherServer.Test.CMS.Comments.MeetupCommentEmotions do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(user user2 user3 meetup)a}
  end

  describe "[emotion in paged article comment]" do
    test "login user should got viewer has emotioned status", ~m(meetup user)a do
      total_count = 0
      page_number = 10
      page_size = 20
  ...
  describe "[basic article comment emotion]" do
    test "comment has default emotions after created", ~m(meetup user)a do
    ...
    test "can make emotion to comment", ~m(meetup user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:meetup, meetup.id, mock_comment(), user)
    ...
    test "can undo emotion to comment", ~m(meetup user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:meetup, meetup.id, mock_comment(), user)
    ...
    test "same user make same emotion to same comment.", ~m(meetup user)a do
      {:ok, parent_comment} = CMS.create_comment(:meetup, meetup.id, mock_comment(), user)
    ...
    test "different user can make same emotions on same comment", ~m(meetup user user2 user3)a do
      {:ok, parent_comment} = CMS.create_comment(:meetup, meetup.id, mock_comment(), user)
    ...
    test "same user can make differcent emotions on same comment", ~m(meetup user)a do
      {:ok, parent_comment} = CMS.create_comment(:meetup, meetup.id, mock_comment(), user)
