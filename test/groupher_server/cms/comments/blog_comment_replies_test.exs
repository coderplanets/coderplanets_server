defmodule GroupherServer.Test.CMS.Comments.BlogCommentReplies do
  @moduledoc false
  ...
  alias CMS.Model.{ArticleComment, Blog}
  ...
  describe "[basic article comment replies]" do
    test "exsit comment can be reply", ~m(blog user user2)a do
      parent_content = "parent comment"
      reply_content = "reply comment"

      {:ok, parent_comment} = CMS.create_article_comment(:blog, blog.id, parent_content, user)
    ...
    test "deleted comment can not be reply", ~m(blog user user2)a do
      parent_content = "parent comment"
      reply_content = "reply comment"
    ...
    test "multi reply should belong to one parent comment", ~m(blog user user2)a do
      parent_content = "parent comment"
    ...
    test "reply to reply inside a comment should belong same parent comment",
      ...
      {:ok, parent_comment} = CMS.create_article_comment(:blog, blog.id, "parent comment", user)
    ...
    test "comment replies only contains @max_parent_replies_count replies", ~m(blog user)a do
    ...
    test "replyed user should appear in article comment participators", ~m(blog user user2)a do
      {:ok, parent_comment} = CMS.create_article_comment(:blog, blog.id, mock_comment(), user)
    ...
    test "replies count should inc by 1 after got replyed", ~m(blog user user2)a do
      {:ok, parent_comment} = CMS.create_article_comment(:blog, blog.id, mock_comment(), user)
  ...
  describe "[paged article comment replies]" do
    test "can get paged replies of a parent comment", ~m(blog user)a do
      {:ok, parent_comment} = CMS.create_article_comment(:blog, blog.id, mock_comment(), user)
    ...
    test "can get reply_to info of a parent comment", ~m(blog user)a do
      page_number = 1
      page_size = 10
