defmodule GroupherServer.Test.CMS.Comments.DrinkComment do
  @moduledoc false
  ...
  alias CMS.Model.{Comment, PinnedComment, Embeds, Drink}
  ...
  setup do
    ...
    {:ok, ~m(community user user2 drink)a}
  end

  describe "[basic article comment]" do
    test "drink are supported by article comment.", ~m(user drink)a do
      {:ok, drink_comment_1} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, drink_comment_2} = CMS.create_comment(:drink, drink.id, mock_comment(), user)

      {:ok, drink} = ORM.find(Drink, drink.id, preload: :comments)

      assert exist_in?(drink_comment_1, drink.comments)
      assert exist_in?(drink_comment_2, drink.comments)
    end

    test "comment should have default meta after create", ~m(user drink)a do
    ...
    test "create comment should update active timestamp of drink", ~m(user drink)a do
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, drink} = ORM.find(Drink, drink.id, preload: :comments)

      assert not is_nil(drink.active_at)
      assert drink.active_at > drink.inserted_at
    end

    test "drink author create comment will not update active timestamp", ~m(community user)a do
      drink_attrs = mock_attrs(:drink, %{community_id: community.id})
      {:ok, drink} = CMS.create_article(community, :drink, drink_attrs, user)
      {:ok, drink} = ORM.find(Drink, drink.id, preload: [author: :user])

      Process.sleep(1000)

      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), drink.author.user)
    ...
    test "old drink will not update active after comment created", ~m(user)a do
      ...
      {:ok, drink} = db_insert(:drink, %{inserted_at: inserted_at})
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, drink} = ORM.find(Drink, drink.id)

      assert drink.active_at |> DateTime.to_date() == DateTime.utc_now() |> DateTime.to_date()
      ...
      assert drink.active_at |> DateTime.to_unix() !== DateTime.utc_now() |> DateTime.to_unix()
    end

    test "comment can be updated", ~m(drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
  ...
  describe "[article comment floor]" do
    test "comment will have a floor number after created", ~m(user drink)a do
      {:ok, drink_comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, drink_comment2} = CMS.create_comment(:drink, drink.id, mock_comment(), user)

      {:ok, drink_comment} = ORM.find(Comment, drink_comment.id)
      {:ok, drink_comment2} = ORM.find(Comment, drink_comment2.id)

      assert drink_comment.floor == 1
      assert drink_comment2.floor == 2
    end
  end

  describe "[article comment participator for drink]" do
    test "drink will have participator after comment created", ~m(user drink)a do
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)

      {:ok, drink} = ORM.find(Drink, drink.id)

      participator = List.first(drink.comments_participants)
      assert participator.id == user.id
    end

    test "psot participator will not contains same user", ~m(user drink)a do
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)

      {:ok, drink} = ORM.find(Drink, drink.id)

      assert 1 == length(drink.comments_participants)
    end

    test "recent comment user should appear at first of the psot participants",
         ~m(user user2 drink)a do
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user2)
  ...
  describe "[article comment upvotes]" do
    test "user can upvote a drink comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "article author upvote drink comment will have flag", ~m(drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, author_user} = ORM.find(User, drink.author.user.id)
    ...
    test "user upvote drink comment will add id to upvoted_user_ids", ~m(drink user)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "user undo upvote drink comment will remove id from upvoted_user_ids",
    ...
    test "user upvote a already-upvoted comment fails", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "upvote comment should inc the comment's upvotes_count", ~m(user user2 drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "user can undo upvote a drink comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      CMS.upvote_comment(comment.id, user)
    ...
    test "user can undo upvote a drink comment with no upvote", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, comment} = CMS.undo_upvote_comment(comment.id, user)
      assert 0 == comment.upvotes_count
  ...
  describe "[article comment fold/unfold]" do
    test "user can fold a comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, drink} = ORM.find(Drink, drink.id)
      assert drink.meta.folded_comment_count == 1
    end

    test "user can unfold a comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      ...
      {:ok, drink} = ORM.find(Drink, drink.id)
      assert drink.meta.folded_comment_count == 0
    end
  end

  describe "[article comment pin/unpin]" do
    test "user can pin a comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, pined_record} = PinnedComment |> ORM.find_by(%{drink_id: drink.id})
      assert pined_record.drink_id == drink.id
    end

    test "user can unpin a comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "pinned comments has a limit for each article", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
  ...
  describe "[article comment report/unreport]" do
    #
    # test "user can report a comment", ~m(user drink)a do
    #   {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    #   {:ok, comment} = ORM.find(Comment, comment.id)
    ...
    #
    # test "user can unreport a comment", ~m(user drink)a do
    #   {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "can undo a report with other user report it too", ~m(user user2 drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "report user < @report_threshold_for_fold will not fold comment", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "report user > @report_threshold_for_fold will cause comment fold", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(user drink)a do
      total_count = 30
      page_size = 10
      thread = :drink
      ...
      Enum.reduce(1..total_count, [], fn _, acc ->
        ...
        {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), new_user)
      ...
      {:ok, results} =
        CMS.paged_comments_participants(thread, drink.id, %{page: 1, size: page_size})

      assert results |> is_valid_pagination?(:raw)
      assert results.total_count == total_count + 1
    end

    test "paged article comments folded flag should be false", ~m(user drink)a do
      total_count = 30
      page_number = 1
      page_size = 35
    ...
    test "paged article comments should contains pinned comments at top position",
      ...
      {:ok, random_comment_1} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, random_comment_2} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    ...
    test "can loaded paged folded comment", ~m(user drink)a do
      total_count = 10
      page_number = 1
      page_size = 20
      ...
      {:ok, paged_comments} =
        CMS.paged_folded_comments(:drink, drink.id, %{page: page_number, size: page_size})
  ...
  describe "[article comment delete]" do
    test "delete comment still exsit in paged list and content is gone", ~m(user drink)a do
      total_count = 10
      page_number = 1
      page_size = 20
    ...
    test "delete comment still update article's comments_count field", ~m(user drink)a do
      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      ...
      assert drink.comments_count == 5

      {:ok, _} = CMS.delete_comment(comment)

      {:ok, drink} = ORM.find(Drink, drink.id)
      assert drink.comments_count == 4
    end

    test "delete comment still delete pinned record if needed", ~m(user drink)a do
      total_count = 10
  ...
  describe "[article comment info]" do
    test "author of the article comment a comment should have flag", ~m(user drink)a do
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      assert not comment.is_article_author

      author_user = drink.author.user
      {:ok, comment} = CMS.create_comment(:drink, drink.id, mock_comment(), author_user)
      assert comment.is_article_author
    end
  end

  describe "[lock/unlock drink comment]" do
    test "locked drink can not be comment", ~m(user drink)a do
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      {:ok, _} = CMS.lock_article_comments(:drink, drink.id)

      {:error, reason} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
      assert reason |> is_error?(:article_comments_locked)

      {:ok, _} = CMS.undo_lock_article_comments(:drink, drink.id)
      {:ok, _} = CMS.create_comment(:drink, drink.id, mock_comment(), user)
    end

    test "locked drink can not by reply", ~m(user drink)a do
