defmodule GroupherServer.Test.CMS.Comments.GuideComment do
  @moduledoc false
  ...
  alias CMS.Model.{Comment, PinnedComment, Embeds, Guide}
  ...
  setup do
    ...
    {:ok, ~m(community user user2 guide)a}
  end

  describe "[basic article comment]" do
    test "guide are supported by article comment.", ~m(user guide)a do
      {:ok, guide_comment_1} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, guide_comment_2} = CMS.create_comment(:guide, guide.id, mock_comment(), user)

      {:ok, guide} = ORM.find(Guide, guide.id, preload: :comments)

      assert exist_in?(guide_comment_1, guide.comments)
      assert exist_in?(guide_comment_2, guide.comments)
    end

    test "comment should have default meta after create", ~m(user guide)a do
    ...
    test "create comment should update active timestamp of guide", ~m(user guide)a do
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, guide} = ORM.find(Guide, guide.id, preload: :comments)

      assert not is_nil(guide.active_at)
      assert guide.active_at > guide.inserted_at
    end

    test "guide author create comment will not update active timestamp", ~m(community user)a do
      guide_attrs = mock_attrs(:guide, %{community_id: community.id})
      {:ok, guide} = CMS.create_article(community, :guide, guide_attrs, user)
      {:ok, guide} = ORM.find(Guide, guide.id, preload: [author: :user])

      Process.sleep(1000)

      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), guide.author.user)
    ...
    test "old guide will not update active after comment created", ~m(user)a do
      ...
      {:ok, guide} = db_insert(:guide, %{inserted_at: inserted_at})
      Process.sleep(1000)
      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, guide} = ORM.find(Guide, guide.id)

      assert guide.active_at |> DateTime.to_date() == DateTime.utc_now() |> DateTime.to_date()
      ...
      assert guide.active_at |> DateTime.to_unix() !== DateTime.utc_now() |> DateTime.to_unix()
    end

    test "comment can be updated", ~m(guide user)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
  ...
  describe "[article comment floor]" do
    test "comment will have a floor number after created", ~m(user guide)a do
      {:ok, guide_comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, guide_comment2} = CMS.create_comment(:guide, guide.id, mock_comment(), user)

      {:ok, guide_comment} = ORM.find(Comment, guide_comment.id)
      {:ok, guide_comment2} = ORM.find(Comment, guide_comment2.id)

      assert guide_comment.floor == 1
      assert guide_comment2.floor == 2
    end
  end

  describe "[article comment participator for guide]" do
    test "guide will have participator after comment created", ~m(user guide)a do
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)

      {:ok, guide} = ORM.find(Guide, guide.id)

      participator = List.first(guide.comments_participants)
      assert participator.id == user.id
    end

    test "psot participator will not contains same user", ~m(user guide)a do
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)

      {:ok, guide} = ORM.find(Guide, guide.id)

      assert 1 == length(guide.comments_participants)
    end

    test "recent comment user should appear at first of the psot participants",
         ~m(user user2 guide)a do
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user2)
  ...
  describe "[article comment upvotes]" do
    test "user can upvote a guide comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "article author upvote guide comment will have flag", ~m(guide user)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, author_user} = ORM.find(User, guide.author.user.id)
    ...
    test "user upvote guide comment will add id to upvoted_user_ids", ~m(guide user)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "user undo upvote guide comment will remove id from upvoted_user_ids",
         ~m(guide user user2)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "user upvote a already-upvoted comment fails", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "upvote comment should inc the comment's upvotes_count", ~m(user user2 guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "user can undo upvote a guide comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      CMS.upvote_comment(comment.id, user)
    ...
    test "user can undo upvote a guide comment with no upvote", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, comment} = CMS.undo_upvote_comment(comment.id, user)
      assert 0 == comment.upvotes_count
  ...
  describe "[article comment fold/unfold]" do
    test "user can fold a comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, guide} = ORM.find(Guide, guide.id)
      assert guide.meta.folded_comment_count == 1
    end

    test "user can unfold a comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      ...
      {:ok, guide} = ORM.find(Guide, guide.id)
      assert guide.meta.folded_comment_count == 0
    end
  end

  describe "[article comment pin/unpin]" do
    test "user can pin a comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, comment} = ORM.find(Comment, comment.id)
      ...
      {:ok, pined_record} = PinnedComment |> ORM.find_by(%{guide_id: guide.id})
      assert pined_record.guide_id == guide.id
    end

    test "user can unpin a comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "pinned comments has a limit for each article", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
  ...
  describe "[article comment report/unreport]" do
    #
    # test "user can report a comment", ~m(user guide)a do
    #   {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    #   {:ok, comment} = ORM.find(Comment, comment.id)
    ...
    #
    # test "user can unreport a comment", ~m(user guide)a do
    #   {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "can undo a report with other user report it too", ~m(user user2 guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "report user < @report_threshold_for_fold will not fold comment", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "report user > @report_threshold_for_fold will cause comment fold", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
  ...
  describe "paged article comments" do
    test "can load paged comments participants of a article", ~m(user guide)a do
      total_count = 30
      page_size = 10
      thread = :guide
      ...
      Enum.reduce(1..total_count, [], fn _, acc ->
        ...
        {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), new_user)
      ...
      {:ok, results} =
        CMS.paged_comments_participants(thread, guide.id, %{page: 1, size: page_size})

      assert results |> is_valid_pagination?(:raw)
      assert results.total_count == total_count + 1
    end

    test "paged article comments folded flag should be false", ~m(user guide)a do
      total_count = 30
      page_number = 1
      page_size = 35
    ...
    test "paged article comments should contains pinned comments at top position",
      ...
      {:ok, random_comment_1} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, random_comment_2} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    ...
    test "can loaded paged folded comment", ~m(user guide)a do
      total_count = 10
      page_number = 1
      page_size = 20
      ...
      {:ok, paged_comments} =
        CMS.paged_folded_comments(:guide, guide.id, %{page: page_number, size: page_size})
  ...
  describe "[article comment delete]" do
    test "delete comment still exsit in paged list and content is gone", ~m(user guide)a do
      total_count = 10
      page_number = 1
      page_size = 20
    ...
    test "delete comment still update article's comments_count field", ~m(user guide)a do
      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, _comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      ...
      assert guide.comments_count == 5

      {:ok, _} = CMS.delete_comment(comment)

      {:ok, guide} = ORM.find(Guide, guide.id)
      assert guide.comments_count == 4
    end

    test "delete comment still delete pinned record if needed", ~m(user guide)a do
      total_count = 10
  ...
  describe "[article comment info]" do
    test "author of the article comment a comment should have flag", ~m(user guide)a do
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      assert not comment.is_article_author

      author_user = guide.author.user
      {:ok, comment} = CMS.create_comment(:guide, guide.id, mock_comment(), author_user)
      assert comment.is_article_author
    end
  end

  describe "[lock/unlock guide comment]" do
    test "locked guide can not be comment", ~m(user guide)a do
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      {:ok, _} = CMS.lock_article_comments(:guide, guide.id)

      {:error, reason} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
      assert reason |> is_error?(:article_comments_locked)

      {:ok, _} = CMS.undo_lock_article_comments(:guide, guide.id)
      {:ok, _} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
    end

    test "locked guide can not by reply", ~m(user guide)a do
      {:ok, parent_comment} = CMS.create_comment(:guide, guide.id, mock_comment(), user)
