defmodule GroupherServer.Test.CMS.Comments.RadarCommentReplies do
  @moduledoc false
  ...
  setup do
    ...
    {:ok, ~m(user user2 radar)a}
  end

  describe "[basic article comment replies]" do
    test "exsit comment can be reply", ~m(radar user user2)a do
    ...
    test "deleted comment can not be reply", ~m(radar user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user)
    ...
    test "multi reply should belong to one parent comment", ~m(radar user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user)
    ...
    test "comment replies only contains @max_parent_replies_count replies", ~m(radar user)a do
    ...
    test "replyed user should appear in article comment participants", ~m(radar user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user)
      {:ok, _} = CMS.reply_comment(parent_comment.id, mock_comment(), user2)

      {:ok, article} = ORM.find(Radar, radar.id)

      assert exist_in?(user, article.comments_participants)
      assert exist_in?(user2, article.comments_participants)
    end

    test "replies count should inc by 1 after got replyed", ~m(radar user user2)a do
      {:ok, parent_comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user)
  ...
  describe "[paged article comment replies]" do
    test "can get paged replies of a parent comment", ~m(radar user)a do
      {:ok, parent_comment} = CMS.create_comment(:radar, radar.id, mock_comment(), user)
    ...
    test "can get reply_to info of a parent comment", ~m(radar user)a do
      page_number = 1
      page_size = 10
