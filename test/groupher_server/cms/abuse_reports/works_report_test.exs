defmodule GroupherServer.Test.CMS.AbuseReports.WorksReport do
  @moduledoc false
  ...
  describe "[article works report/unreport]" do
    test "list article reports should work", ~m(community user user2 works_attrs)a do
      ...
      assert report.article.id == works.id
      assert report.article.thread == "WORKS"
    end

    test "report a works should have a abuse report record", ~m(community user works_attrs)a do
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user)
      ...
      assert works.meta.reported_count == 1
      assert user.id in works.meta.reported_user_ids
    end

    test "can undo a report", ~m(community user works_attrs)a do
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user)
      ...
      {:ok, works} = ORM.find(Works, works.id)
      assert user.id not in works.meta.reported_user_ids
    end

    test "can undo a existed report", ~m(community user user2 works_attrs)a do
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user2)
      {:ok, _report} = CMS.undo_report_article(:works, works.id, user)
      ...
      assert user2.id in works.meta.reported_user_ids
      assert user.id not in works.meta.reported_user_ids
    end

    test "can undo a report with other user report it too",
         ~m(community user user2 works_attrs)a do
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user)
      {:ok, _report} = CMS.report_article(:works, works.id, "reason", "attr_info", user2)
    ...
    test "same user can not report a comment twice", ~m(community works_attrs user)a do
      {:ok, works} = CMS.create_article(community, :works, works_attrs, user)
