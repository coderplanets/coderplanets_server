defmodule MastaniServer.Test.PostComment do
  ...
  describe "[comment CURD]" do
    ...
    test "can reply a comment, and reply should be in comment replies list", ~m(comment user)a do
      ...
      {:ok, reply} = CMS.reply_comment(:post, comment.id, %{body: reply_body}, user)
    ...
    test "comment reply can be list one-by-one --> by replied user", ~m(comment)a do
      ...
      {:ok, _} = CMS.reply_comment(:post, comment.id, %{body: "reply by user1"}, user1)

      {:ok, _} = CMS.reply_comment(:post, comment.id, %{body: "reply by user2"}, user2)

      {:ok, _} = CMS.reply_comment(:post, comment.id, %{body: "reply by user3"}, user3)
