defmodule MastaniServer.Test.JobReactionsTest do
  ...
  describe "[cms job star/favorite reaction]" do
    test "star and undo star reaction to job", ~m(user job_attrs)a do
      {:ok, job} = CMS.create_content(:job, %Accounts.User{id: user.id}, job_attrs)

      {:ok, _} = CMS.reaction(:job, :star, job.id, %Accounts.User{id: user.id})
      ...
      {:ok, _} = CMS.undo_reaction(:job, :star, job.id, %Accounts.User{id: user.id})
    ...
    test "favorite and undo favorite reaction to job", ~m(user job_attrs)a do
      {:ok, job} = CMS.create_content(:job, %Accounts.User{id: user.id}, job_attrs)

      {:ok, _} = CMS.reaction(:job, :favorite, job.id, %Accounts.User{id: user.id})
      {:ok, reaction_users} = CMS.reaction_users(:job, :favorite, job.id, %{page: 1, size: 1})
      reaction_users = reaction_users |> Map.get(:entries)
      assert 1 == reaction_users |> Enum.filter(fn ruser -> user.id == ruser.id end) |> length

      {:ok, _} = CMS.undo_reaction(:job, :favorite, job.id, %Accounts.User{id: user.id})
